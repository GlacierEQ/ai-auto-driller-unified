// ==UserScript==
// @name         Kimi Auto-Driller Unified — MAX Edition
...
(function() {
    ...
    const SEL = {
        approve: ['button:has-text("允许")','button:has-text("Allow")','button:has-text("Accept")','button:has-text("确认")'],
        input: ['textarea','div[contenteditable="true"][role="textbox"]','div[contenteditable="true"]'],
        submit: ['button[aria-label*="发送" i]','button[aria-label*="Send" i]','button[type="submit"]','button:has(svg)'],
        response: ['[class*="message"]','[class*="response"]','.markdown','[class*="content"]'],
        loading: ['[class*="loading"]','[class*="generating"]','[class*="typing"]','[aria-busy="true"]']
    };
    const log = (...a) => { if (CONFIG.debug) console.log('[KIMI-MAX]', ...a); };
    ...
    const extractTopic = (text) => {
        ...
        const stops = new Set(['the','a','an','and','or','but','in','on','at','to','for','of','with','by','from','as','is','was','are','this','that','的','了','在','是','我','有','和','就','不','人','都','一','一个','上','也','很','到','说','要','去','你','会','着','没有','看','好','自己','这']);
        const words = text.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff\s]/g,'').split(/\s+/).filter(w => w.length>2 && !stops.has(w)).slice(0,3);
    ...
    const ui = {
        ...
        events(){
            ...
            document.addEventListener('keydown',()=>{STATE.lastUserActivity=Date.now();});document.addEventListener('input',()=>{STATE.lastUserActivity=Date.now();});

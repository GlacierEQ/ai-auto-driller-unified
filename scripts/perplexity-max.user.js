// ==UserScript==
// @name         Perplexity Auto-Driller Unified — MAX Edition
// @namespace    https://github.com/GlacierEQ
// @version      4.0.0
// @description  UNIFIED: 8 weighted categories, typing sim, auto-approve, draggable UI, session export.
// @author       GlacierEQ
...
(function() {
    ...
    const CONFIG = {
        ...
        drillInterval: GM_getValue('drillInterval', 8000), typingSpeed: GM_getValue('typingSpeed', 50),
    ...
    const DRILL_PATTERNS = {
        clarification: ["Can you elaborate on {topic} with examples?","What are the nuances of {topic}?","Could you break down {topic} into components?"],
        depth: ["What are the underlying principles of {topic}?","How has {topic} evolved?","What are the theoretical foundations of {topic}?"],
        practical: ["What are real-world applications of {topic}?","How can I implement {topic}?","What tools are best for {topic}?"],
        comparative: ["How does {topic} compare to alternatives?","What are pros and cons of {topic}?","What makes {topic} different?"],
        future: ["What's the future outlook for {topic}?","What trends are shaping {topic}?","What's next for {topic}?"],
        technical: ["What are best practices for {topic}?","What are common pitfalls with {topic}?","How do experts optimize {topic}?"],
        problem_solving: ["What problems does {topic} solve?","What are the limitations of {topic}?","How can I troubleshoot {topic}?"],
        integration: ["How does {topic} integrate with existing systems?","What dependencies does {topic} require?","What's the learning curve for {topic}?"]
    };
    const WEIGHTS = [1,3,4,2,2,3,3,2];
    const SEL = {
        approve: ['button:has-text("Approve")','button:has-text("Allow")','button:has-text("Yes")','button[aria-label*="pprove" i]'],
        input: ['textarea[placeholder*="Ask" i]','textarea[role="textbox"]','textarea'],
        submit: ['button[type="submit"]','button[aria-label*="Submit" i]'],
        response: ['[class*="answer"]','[class*="response"]','article','.prose'],
        loading: ['[aria-busy="true"]','[class*="loading"]','[class*="generating"]']
    };
    const log = (...a) => { if (CONFIG.debug) console.log('[PPLX-MAX]', ...a); };
    ...
    const isIdle = () => !document.querySelector('[aria-busy="true"]') && !document.querySelector('[class*="generating"]');
    ...
    GM_registerMenuCommand('Toggle Auto-Approve',()=>{CONFIG.autoAccept=!CONFIG.autoAccept;saveConfig();});

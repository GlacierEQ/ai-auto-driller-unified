// ==UserScript==
// @name         Gemini Auto-Driller Unified — MAX Edition
...
(function() {
    ...
    const SEL = {
        approve: ['button:has-text("Allow")','button:has-text("Continue")'],
        input: ['div[contenteditable="true"][role="textbox"]','textarea'],
        submit: ['button[aria-label*="Send" i]','button[aria-label*="Submit" i]'],
        response: ['model-response','[class*="response"]','.markdown'],
        loading: ['[class*="loading"]','[class*="generating"]']
    };
    const log = (...a) => { if (CONFIG.debug) console.log('[Gemini-MAX]', ...a); };

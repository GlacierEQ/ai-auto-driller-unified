// ==UserScript==
// @name         Cohere Auto-Driller Unified — MAX Edition
...
(function() {
    ...
    const SEL = {
        approve: ['button:has-text("Allow")','button:has-text("Accept")','button:has-text("Confirm")'],
        input: ['textarea','div[contenteditable="true"][role="textbox"]','div[contenteditable="true"]'],
        submit: ['button[aria-label*="Send" i]','button[type="submit"]','button:has(svg)'],
        response: ['[class*="message"]','[class*="response"]','.markdown','[class*="prose"]'],
        loading: ['[class*="loading"]','[class*="generating"]','[class*="spinner"]','[aria-busy="true"]']
    };
    const log = (...a) => { if (CONFIG.debug) console.log('[COHERE-MAX]', ...a); };
